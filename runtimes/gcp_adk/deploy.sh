...
gcloud run jobs deploy "$JOB" --source . --region "$REGION" --project "$PROJECT" \
  --set-env-vars="GOOGLE_GENAI_USE_VERTEXAI=TRUE,GOOGLE_CLOUD_PROJECT=$PROJECT,GOOGLE_CLOUD_LOCATION=global,DUCKFLEET_PROJECT_ID=$PROJECT,DUCKFLEET_REGION=$REGION,DUCKFLEET_MODEL_FAST=gemini-3.5-flash,DUCKFLEET_MODEL_STRONG=gemini-3.5-flash,DUCKFLEET_REPLAY=$REPLAY,DUCKFLEET_BIGQUERY_DATASET=duckfleet,DUCKFLEET_GMAIL_SENDER=$SENDER,DUCKFLEET_NOTIFY_EMAIL=$NOTIFY_EMAIL" \
